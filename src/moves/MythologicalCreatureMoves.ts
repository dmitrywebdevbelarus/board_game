...
import type { CanBeUndef, CanBeVoid, DwarfCard, GetMoveArgument, InvalidMove, MoveContext, MoveFn, MythologicalCreatureCard, MythologicalCreatureCommandZoneCard, MythologicalCreatureDeckForSkymirCardId, PlayerCoinId, PlayerStack, PublicPlayer, StackCards } from "../typescript/interfaces";
...
export const ActivateGodAbilityMove: MoveFn<GetMoveArgument<CardMoveNames.ActivateGodAbilityMove>> = (
...
// TODO godName: GodNames => string and asserts it value if no other strings can be valid in moves!?
export const NotActivateGodAbilityMove: MoveFn<GetMoveArgument<ButtonMoveNames.NotActivateGodAbilityMove>> = (
    { G, ctx, playerID, events, ...rest }: MoveContext,
    godName: GodNames,
): CanBeVoid<InvalidMove> => {
    const isValidMove: boolean = IsValidMove(
        { G, ctx, playerID, events, ...rest },
        TavernsResolutionWithSubStageNames.ActivateGodAbilityOrNot,
    ...
    const stackDwarfCard: CanBeUndef<StackCards> = stack.card;
 ...
 * @returns
 */
export const ChooseCoinValueForHrungnirUpgradeMove:
    MoveFn<GetMoveArgument<CoinMoveNames.ChooseCoinValueForHrungnirUpgradeMove>> = (
        { G, playerID, ...rest }: MoveContext,
        coinId: PlayerCoinId,
    ): CanBeVoid<InvalidMove> => {
        ...
        let nextCoinId: CanBeUndef<PlayerCoinId> = stack.coinId;
 ...
 * @returns
 */
export const ClickCardNotGiantAbilityMove: MoveFn<GetMoveArgument<CardMoveNames.ClickCardNotGiantAbilityMove>> = (
    { G, playerID, ...rest }: MoveContext,
    dwarfCard: DwarfCard,
): CanBeVoid<InvalidMove> => {
    const isValidMove: boolean = IsValidMove(
 ...
 * @returns
 */
export const ClickGiantAbilityNotCardMove: MoveFn<GetMoveArgument<CardMoveNames.ClickGiantAbilityNotCardMove>> = (
    { G, playerID, ...rest }: MoveContext,
    card: DwarfCard,
): CanBeVoid<InvalidMove> => {
    const isValidMove: boolean = IsValidMove(
    ...
    switch (card.playerSuit) {
        ...
        case SuitNames.miner:
            if (CheckPlayerHasBuff(
                ...
                buffName = GiantBuffNames.PlayerHasActiveGiantHrungnir;
                AddPlusTwoValueToAllCoinsAction(
                    { G, ...rest },
                    playerID,
                );
            } else {
 ...
 * @returns
 */
export const ChooseSuitOlrunMove: MoveFn<GetMoveArgument<SuitMoveNames.ChooseSuitOlrunMove>> = (
    { G, playerID, ...rest }: MoveContext,
    suit: SuitNames,
): CanBeVoid<InvalidMove> => {
    const isValidMove: boolean = IsValidMove(
 ...
 * @returns
 */
export const GetMythologyCardMove: MoveFn<GetMoveArgument<CardMoveNames.GetMythologyCardMove>> = (
    { G, playerID, ...rest }: MoveContext,
    cardId: MythologicalCreatureDeckForSkymirCardId,
): CanBeVoid<InvalidMove> => {
    ...
    const mythologyCard: CanBeUndef<MythologicalCreatureCard> =
        G.mythologicalCreatureDeckForSkymir.splice(cardId, 1)[0];
