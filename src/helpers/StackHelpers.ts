...
import type { CanBeUndef, CardsHasStack, Context, PickCardValidatorNamesKeyof, PlayerID, PublicPlayer, Stack, ValidatorsConfig } from "../typescript/interfaces";
 ...
 * @returns
 */
export const AddActionsToStack = (
    { G, ctx, ...rest }: Context,
    playerID: PlayerID,
    stack?: Stack[],
    card?: CardsHasStack,
    ...
    if (stack !== undefined) {
        if (card !== undefined && `validators` in card) {
            const validators: CanBeUndef<ValidatorsConfig> = card.validators;
            if (validators !== undefined) {
                let validator: PickCardValidatorNamesKeyof,
                    _exhaustiveCheck: never;
                for (validator in validators) {
                    if (validator === PickCardValidatorNames.pickDiscardCardToStack) {
                        isValid = IsCanPickPickDiscardCardToStack(
                            { G, ctx, ...rest },
                            card,
                        );
                    } else if (validator === PickCardValidatorNames.pickCampCardToStack) {
                        isValid = IsCanPickPickCampCardToStack(
                            { G, ctx, ...rest },
                            card,
                        );
                    } else {
        ...
        if (isValid) {
            for (let i = 0; i < stack.length; i++) {
                const stackI: CanBeUndef<Stack> = stack[i];
                if (stackI === undefined) {
