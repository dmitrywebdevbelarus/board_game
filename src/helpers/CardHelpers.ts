...
import type { AllCard, AllPickedCard, CanBeUndef, Context, MythologicalCreatureCommandZoneCard, PlayerBoardCard, PlayerID, PublicPlayer } from "../typescript/interfaces";
 ...
 * @returns
 */
export const AddCardToPlayerBoardCards = (
    { G, ...rest }: Context,
    playerID: PlayerID,
            ...
            case CardRusNames.ArtefactPlayerCard:
                if (G.expansions.Idavoll.active) {
                    ...
                    if (!(card.name === HeroNames.Thrud || HeroNames.Ylud)) {
                        if (CheckIfRecruitedCardEqualSuitIdForOlrun(
                            { G, ...rest },
                            playerID,
                            card.suit,
                        )) {
    ...
    switch (card.type) {
        ...
        case CardRusNames.ArtefactPlayerCard:
            ...
            AddDataToLog(
                { G, ...rest },
                LogNames.Public,
                `Игрок '${player.nickname}' выбрал карту '${card.type}' '${card.name}' во фракцию '${suitsConfig[card.suit].suitName}'.`,
            );
            break;
        default:
 ...
 * @returns
 */
const AddMythologicalCreatureCardToPlayerCommandZone = (
    { G, ...rest }: Context,
    playerID: PlayerID,
    card: MythologicalCreatureCommandZoneCard,
): void => {
    const player: CanBeUndef<PublicPlayer> = G.publicPlayers[playerID];
    if (player === undefined) {
    ...
    AddDataToLog(
        { G, ...rest },
        LogNames.Public,
        `Игрок '${player.nickname}' выбрал карту '${card.type}' '${card.name}' в командную зону карт Idavoll.`,
    );
};
 ...
 */
export const AddAnyCardToPlayerActions = (
    { G, ctx, ...rest }: Context,
    playerID: PlayerID,
    pickedCard: AllPickedCard,
): void => {
    const player: CanBeUndef<PublicPlayer> = G.publicPlayers[playerID];
    if (player === undefined) {
        return ThrowMyError(
    ...
    let finalPickedCard: AllCard = pickedCard,
        startGiant = false,
    ...
    switch (pickedCard.type) {
        ...
        case CardRusNames.MercenaryCard:
            finalPickedCard = AddCampCardToCards(
                { G, ctx, ...rest },
                playerID,
                pickedCard,
            );
        ...
        case CardRusNames.HeroCard:
            finalPickedCard = AddHeroToPlayerCards(
                { G, ctx, ...rest },
                playerID,
                pickedCard,
            );
                        ...
                        case SuitNames.blacksmith:
                            if (CheckPlayerHasBuff(
                                ...
                                AddActionsToStack(
                                    { G, ctx, ...rest },
                                    playerID,
                                    [AllStackData.activateGiantAbilityOrPickCard(
                                        GiantNames.Thrivaldi,
                                        pickedCard,
                                    )],
                                );
                                startGiant = true;
        ...
        case CardRusNames.RoyalOfferingCard:
            if (!(G.expansions.Idavoll.active && ctx.phase === PhaseNames.TavernsResolution
                ...
                AddDataToLog(
                    { G, ctx, ...rest },
                    LogNames.Public,
                    `Игрок '${player.nickname}' выбрал карту '${pickedCard.type}' '${pickedCard.name}'.`,
                );
                DiscardCurrentCard(
                    { G, ctx, ...rest },
                    pickedCard,
                ...
                AddDataToLog(
                    { G, ctx, ...rest },
                    LogNames.Game,
                    `Карта '${pickedCard.type}' '${pickedCard.name}' убрана в сброс после применения её эффекта.`,
                );
            }
            break;
        case CardRusNames.GodCard:
        ...
        case CardRusNames.ValkyryCard:
            if (G.expansions.Idavoll.active) {
                AddMythologicalCreatureCardToPlayerCommandZone(
                    { G, ctx, ...rest },
                    playerID,
                    pickedCard,
                );
    ...
    if (`stack` in pickedCard && pickedCard.stack !== undefined) {
        if (G.mode === GameModeNames.Solo && playerID === PlayerIdForSoloGameNames.SoloBotPlayerId) {
            AddActionsToStack(
                { G, ctx, ...rest },
                playerID,
                pickedCard.stack.soloBot ?? pickedCard.stack.player,
                pickedCard,
            );
