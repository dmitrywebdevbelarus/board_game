...
import type { ActionFunctionWithoutParams, ActivePlayersArgValue, CanBeUndef, Context, PlayerHandCoins, PlayerID, PrivatePlayer, PublicPlayer, PublicPlayerCoin, Stack } from "../typescript/interfaces";
 ...
 * @returns
 */
export const DiscardTradingCoinAction: ActionFunctionWithoutParams = (
    { G, ...rest }: Context,
    playerID: PlayerID,
): void => {
    ...
    AddDataToLog(
        { G, ...rest },
        LogNames.Game,
        `Игрок '${player.nickname}' сбросил монету активирующую обмен.`,
    );
};
 ...
 * @returns
 */
export const FinishOdroerirTheMythicCauldronAction: ActionFunctionWithoutParams = (
    { G }: Context,
): void => {
 ...
 * @returns
 */
export const StartDiscardSuitCardAction: ActionFunctionWithoutParams = (
    { G, ctx, events, ...rest }: Context,
): void => {
    // TODO Can i rework it!?
    const value: ActivePlayersArgValue = {} as ActivePlayersArgValue;
    let results = 0;
    ...
    if (results) {
        ...
        AddDataToLog(
            { G, ctx, events, ...rest },
            LogNames.Game,
            `Нет игроков с картами во фракции '${SuitNames.warrior}'.`,
        );
    }
};
 ...
 * @returns
 */
export const StartVidofnirVedrfolnirAction: ActionFunctionWithoutParams = (
    { G, ...rest }: Context,
    playerID: PlayerID,
): void => {
    const player: CanBeUndef<PublicPlayer> = G.publicPlayers[playerID],
    ...
    if (isStart) {
        ...
        for (let j: number = G.tavernsNum; j < player.boardCoins.length; j++) {
            ...
            let boardCoin: PublicPlayerCoin;
            if (G.mode === GameModeNames.Multiplayer) {
