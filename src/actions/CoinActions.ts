...
import type { CanBeUndef, Coin, CoinCanBeUpgradedByValue, CoinUpgradeBuffValue, Context, PlayerBoardCoins, PlayerCoinId, PlayerHandCoins, PlayerID, PrivatePlayer, PublicPlayer, PublicPlayerCoin, RoyalCoin, UpgradableCoin } from "../typescript/interfaces";
 ...
 * @returns
 */
export const UpgradeCoinAction = (
    { G, ...rest }: Context,
    playerID: PlayerID,
    isTrading: boolean,
    value: CoinCanBeUpgradedByValue,
    upgradingCoinId: PlayerCoinId,
    type: CoinNames,
): void => {
    ...
    let upgradingCoin: UpgradableCoin,
        _exhaustiveCheck: never;
    const handCoin: PublicPlayerCoin = handCoins[upgradingCoinId],
        boardCoin: PublicPlayerCoin = boardCoins[upgradingCoinId];
    switch (type) {
        case CoinNames.Hand:
            if (handCoin === null) {
    ...
    const buffValue: CoinUpgradeBuffValue =
        CheckPlayerHasBuff(
            { G, ...rest },
            playerID,
            HeroBuffNames.UpgradeCoin,
        ) ? 2 : 0,
        newValue: number = upgradingCoin.value + value + buffValue;
    AssertCoinUpgradePossibleMaxValue(newValue);
    let upgradedCoin: Coin = null;
    if (G.royalCoins.length) {
        const lastRoyalCoin: CanBeUndef<RoyalCoin> = G.royalCoins[G.royalCoins.length - 1];
        ...
        if (newValue > lastRoyalCoin.value) {
            ...
            RemoveCoinFromMarket(
                { G, ...rest },
                G.royalCoins.length - 1,
            );
        } else {
            for (let i = 0; i < G.royalCoins.length; i++) {
                const royalCoin: CanBeUndef<RoyalCoin> = G.royalCoins[i];
    ...
    AddDataToLog(
        { G, ...rest },
        LogNames.Game,
        `Начато обновление монеты с ценностью '${upgradingCoin.value}' на '+${value}'.`,
    );
    AddDataToLog(
        { G, ...rest },
        LogNames.Private,
        `Начато обновление монеты c ID '${upgradingCoinId}' с типом '${type}' с initial '${IsInitialCoin(upgradingCoin) ? true : false}' с ценностью '${upgradingCoin.value}' на '+${value}' с новым значением '${newValue}' с итоговым значением '${upgradedCoin.value}'.`,
    );
    ...
    if ((((G.mode === GameModeNames.Basic || G.mode === GameModeNames.Multiplayer)
        ...
        || ((G.mode === GameModeNames.Basic || G.mode === GameModeNames.Multiplayer)
            ...
            ) && type === CoinNames.Board && isTrading)) {
        if (isTrading) {
        ...
        AddDataToLog(
            { G, ...rest },
            LogNames.Public,
            `Монета с ценностью '${upgradedCoin.value}' вернулась на руку игрока '${player.nickname}'.`,
        );
        ...
        AddDataToLog(
            { G, ...rest },
            LogNames.Public,
            `Монета с ценностью '${upgradedCoin.value}' вернулась на поле игрока '${player.nickname}'.`,
        );
    }
    if (!IsInitialCoin(upgradingCoin)) {
        ...
        for (let i = 0; i < G.royalCoins.length; i++) {
            returningIndex = i;
            const royalCoinReturn: CanBeUndef<RoyalCoin> = G.royalCoins[i];
        ...
        AddDataToLog(
            { G, ...rest },
            LogNames.Game,
            `Монета с ценностью '${upgradingCoin.value}' вернулась на рынок.`,
        );
    }
