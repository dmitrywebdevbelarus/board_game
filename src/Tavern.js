...
export const DiscardCardIfTavernHasCardFor2Players = ({ G, ctx, ...rest }) => {
    ...
    AddDataToLog({ G, ctx, ...rest }, LogNames.Game, `Лишняя карта из текущей таверны ${tavernsConfig[G.currentTavern].name} должна быть убрана в сброс при игре ${(G.mode === GameModeNames.Solo || G.mode === GameModeNames.SoloAndvari) ? `в соло режиме` : `на двух игроков в игре`}.`);
 ...
 */
export const DiscardCardFromTavern = ({ G, ...rest }) => {
    const tavernCardId = G.taverns[G.currentTavern].findIndex((card) => card !== null);
    if (tavernCardId === -1) {
        return ThrowMyError({ G, ...rest }, ErrorNames.DoNotDiscardCardFromCurrentTavernIfNoCardInTavern, G.currentTavern);
    }
    AssertTavernCardId(tavernCardId);
    DiscardConcreteCardFromTavern({ G, ...rest }, tavernCardId);
};
/**
 ...
 * @returns Сброшена ли карта из таверны.
 */
export const DiscardConcreteCardFromTavern = ({ G, ...rest }, tavernCardId) => {
    const discardedCard = RemoveCardFromTavern({ G, ...rest }, tavernCardId);
    DiscardCurrentCard({ G, ...rest }, discardedCard);
    AddDataToLog({ G, ...rest }, LogNames.Game, `Карта '${discardedCard.type}' '${discardedCard.name}' из таверны ${tavernsConfig[G.currentTavern].name} убрана в сброс.`);
};
/**
 ...
 * @returns
 */
export const RefillTaverns = ({ G, ...rest }) => {
    for (let t = 0; t < G.tavernsNum; t++) {
        ...
        if (G.expansions.Idavoll.active && G.tierToEnd === 2 && G.round < 3 && t === 1) {
            ...
            refillDeck = GetCardsFromSecretDwarfDeck({ G, ...rest }, tier, 0, G.drawSize);
        }
        AssertTavernIndex(t);
        ...
        AddDataToLog({ G, ...rest }, LogNames.Game, `Таверна ${tavernsConfig[t].name} заполнена новыми картами.`);
    }
    AddDataToLog({ G, ...rest }, LogNames.Game, `Все таверны заполнены новыми картами.`);
};
/**
 * <h3>Конфиг таверн.</h3>
 * <p>Применения:</p>
